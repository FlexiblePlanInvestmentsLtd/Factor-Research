...
def plot_R2_bar(R_df, type):
    ...
    labels = ['K=1', 'K=2', 'K=3', 'K=4', 'K=5', 'K=6', 'K=9', 'K=12', 'K=15']
    ...
    ax.bar(x - width , FF, width, label='FF', color=plt.get_cmap('OrRd')(np.linspace(0, 1, 6)[1]))
    ax.bar(x , PCA, width, label='PCA', color=plt.get_cmap('OrRd')(np.linspace(0, 1, 6)[2]))
    ax.bar(x + width , CA1, width, label='CA1', color=plt.get_cmap('OrRd')(np.linspace(0, 1, 6)[3]))
    ax.bar(x + width*2 , CA2, width, label='CA2', color=plt.get_cmap('OrRd')(np.linspace(0, 1, 6)[4]))
    ax.bar(x + width*3 , CA3, width, label='CA3', color=plt.get_cmap('OrRd')(np.linspace(0, 1, 6)[5]))
...
def plot_R2_table(R_df, type):
    ...
    R_df.columns = ['Model', 'K=1', 'K=2', 'K=3', 'K=4', 'K=5', 'K=6', 'K=9', 'K=12', 'K=15']
...
if __name__=="__main__":
    CAs = ["CA1_1", "CA1_2", "CA1_3", "CA1_4", "CA1_5", "CA1_6", "CA1_9", "CA1_12", "CA1_15"\
           , "CA2_1", "CA2_2", "CA2_3", "CA2_4", "CA2_5", "CA2_6", "CA2_9", "CA2_12", "CA2_15"\
            , "CA3_1", "CA3_2", "CA3_3", "CA3_4", "CA3_5", "CA3_6", "CA3_9", "CA3_12", "CA3_15"]
    FFs = ["FF_1", "FF_2", "FF_3", "FF_4", "FF_5", "FF_6", "FF_9", "FF_12", "FF_15"]
    PCAs = ["PCA_1", "PCA_2", "PCA_3", "PCA_4", "PCA_5", "PCA_6", "PCA_9", "PCA_12", "PCA_15"]
    models = FFs + PCAs + CAs
    ...
    ## Save R^2 tables
    R_total_df = pd.DataFrame(np.array(total_R2).reshape(-1, 9), columns = ['K=1', 'K=2', 'K=3', 'K=4', 'K=5', 'K=6', 'K=9', 'K=12', 'K=15'], index=['FF', 'PCA','CA1', 'CA2', 'CA3'])
    R_pred_df = pd.DataFrame(np.array(predict_R2).reshape(-1, 9), columns = ['K=1', 'K=2', 'K=3', 'K=4', 'K=5', 'K=6', 'K=9', 'K=12', 'K=15'], index=['FF', 'PCA', 'CA1', 'CA2', 'CA3'])
