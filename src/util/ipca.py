    ...
    def predict(self, X=None, indices=None, W=None, mean_factor=False,
        ...
        if data_type == "panel":
            ...
            if W is not None:
                ...
                X, indices, metad = self.X, self.x, self.metad
                N, L, T = metad["N"], metad["L"], metad["T"]
                Q, W, val_obs = _build_portfolio(X, None, indices, metad)
    ...
    def _fit_ipca(self, X=None, y=None, indices=None, PSF=None, Q=None,
        ...
        if Gamma is None or Factors is None:
            try:
                ...
                print("Failed SVD")
                m,n = Q.shape
                Gamma_Old = ortho_group.rvs(m)[:, :n]  # Random orthogonal matrix
                s = abs(np.random.rand(min(m, n)))  # Random singular values
                v = ortho_group.rvs(n)  # Random orthogonal matrix for v
...
def _build_portfolio(X, y, indices, metad):
    ...
    if y is None:
        ...
        # condArray = np.empty(T)
        # stock_Count = np.empty(T)
        for t in range(T):
            ixt = (indices[:, 1] == t)
            val_obs[t] = np.sum(ixt)
            # val_obs[t] = np.count_nonzero(~np.isnan(y[ixt]))
            # X[ixt, :] = np.nan_to_num(X[ixt, :],0.0)
            # y[ixt] = np.nan_to_num(y[ixt],0.0)
            Q[:, t] = (X[ixt, :].T.dot(y[ixt])/val_obs[t]).squeeze()
            # print(Q.shape,np.isnan(Q[:,t]).any(),t)
            # if np.isnan(Q[:,t]).any():
            #     print('Issue')
            # 
            # stock_Count[]
            ...
            # try:
            #     condArray[t] = np.linalg.det(W[:, :, t] )
            # except:
            #     print('Time %d has issue'%t)
