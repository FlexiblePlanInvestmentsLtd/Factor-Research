...
class BuildFeatures:
    ...
    def build_master_index(self):
        self.balance_sheet = (
            (pd.DataFrame(self.data["Financials"]["Balance_Sheet"]["quarterly"]).T).set_index("filing_date").iloc[:, 2:].astype(float)
        )
        ...
        self.income_statement = (
            (pd.DataFrame(self.data["Financials"]["Income_Statement"]["quarterly"]).T).set_index("filing_date").iloc[:, 2:].astype(float)
        )
        ...
        self.cash_flow_staement = (
            (pd.DataFrame(self.data["Financials"]["Cash_Flow"]["quarterly"]).T).set_index("filing_date").iloc[:, 2:].astype(float)
        )
    ...
    def process_financial_statements(self):
        ...
        self.fundamental_features.loc[:, "EPS"] = (
            self.income_statement.loc[:, "netIncome"] / self.balance_sheet.loc[:, "commonStockSharesOutstanding"]
        )
        self.fundamental_features.loc[:, "Free Cash Flow to Firm"] = (
            self.income_statement.loc[:, "ebitda"]
            - self.income_statement.loc[:, "incomeTaxExpense"]
            - self.cash_flow_staement["capitalExpenditures"]
            - self.cash_flow_staement["changeInWorkingCapital"]
        )
        ...
        self.fundamental_features.loc[:, "Dividend Payout Ratio"] = (
            self.cash_flow_staement.loc[:, "dividendsPaid"].fillna(0) / self.income_statement.loc[:, "netIncome"]
        )
        ...
        self.fundamental_features.loc[:, "sustainable growth ratio"] = (
            self.fundamental_features.loc[:, "retention ratio"] * self.fundamental_features.loc[:, "ROE"]
        )
        ...
        for i_feature in self.fundamental_features.columns:
            for i_lookback in lookbacks:
                if i_feature != "Dividend Payout Ratio" and i_feature != "Buybacks":
                    self.fundamental_features[f"{i_feature}_{i_lookback}"] = (
                        self.fundamental_features[i_feature].astype(float) / self.fundamental_features[i_feature].astype(float).shift(-i_lookback)
                    ) - 1
